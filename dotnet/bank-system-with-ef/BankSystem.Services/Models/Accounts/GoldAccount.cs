...
public class GoldAccount : BankAccount
{
    ...
    public GoldAccount(AccountOwner owner, string currencyCode, Func<string> generator)
        : base(owner, currencyCode, new DelegateNumberGenerator(generator))
    {
    }

    public GoldAccount(AccountOwner owner, string currencyCode, Func<string> generator, decimal initialBalance)
    ...
    {
        return (int)((this.Balance / 50m) + (amount / 2.269m));
    }

    protected override int CalculateWithdrawRewardPoints(decimal amount)
    {
        return (int)((this.Balance / 50m) + (amount / 0.98m));
    }
