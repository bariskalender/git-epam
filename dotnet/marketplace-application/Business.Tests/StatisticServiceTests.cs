...
public class StatisticServiceTests
{
    ...
    [TestCase(4)]
    [TestCase(5)]
    public async Task GetMostPopularProductsAsync_ValidCount_ReturnsMostPopularProductsOrderedBySalesCount(int productsCount)
    {
        //arrange
        var mockUnitOfWork = new Mock<IUnitOfWork>();
        _ = mockUnitOfWork.Setup(x => x.ReceiptDetailRepository.GetAllWithDetailsAsync()).ReturnsAsync(ReceiptDetailEntities.AsEnumerable());
    ...
    [TestCase(2)]
    [TestCase(3)]
    public async Task GetMostValuableCustomersAsync_ValidCount_ReturnsCustomersOrderedByAllReceiptsSum(int customerCount)
    {
        //arrange
        var mockUnitOfWork = new Mock<IUnitOfWork>();
        _ = mockUnitOfWork.Setup(x => x.ReceiptRepository.GetAllWithDetailsAsync()).ReturnsAsync(ReceiptEntities.AsEnumerable());
        var statisticService = new StatisticService(mockUnitOfWork.Object, UnitTestBusinessHelper.CreateMapperProfile());
        ...
        _ = actual.Should().BeInDescendingOrder(c => c.ReceiptSum).And.BeEquivalentTo(expected);
    }

    [Test]
    public async Task GetCustomersMostPopularProductsAsync_ValidCustomerId_ReturnsCustomersTopProductsOrderedBySalesCount()
    {
        //arrange
        var mockUnitOfWork = new Mock<IUnitOfWork>();
        _ = mockUnitOfWork.Setup(x => x.ReceiptRepository.GetAllWithDetailsAsync()).ReturnsAsync(ReceiptEntities.AsEnumerable());
        ...
        //assert
        _ = actual.Should().BeEquivalentTo(ExpectedCustomersMostPopularProducts, options => options.Excluding(x => x.ReceiptDetailIds));
    }
    ...
    [TestCase(3, 795)]
    [TestCase(55, 0)]
    public async Task GetIncomeOfCategoryInPeriod_ValidCategoryAndPeriod_ReturnsSumOfCategoryProductsSalesInPeriod(int categoryId, decimal sum)
    {
        var mockUnitOfWork = new Mock<IUnitOfWork>();
        _ = mockUnitOfWork.Setup(x => x.ReceiptRepository.GetAllWithDetailsAsync()).ReturnsAsync(ReceiptEntities.AsEnumerable());
        var statisticService = new StatisticService(mockUnitOfWork.Object, UnitTestBusinessHelper.CreateMapperProfile());
