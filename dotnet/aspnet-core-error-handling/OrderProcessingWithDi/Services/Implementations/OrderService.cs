...
public class OrderService : IOrderService
{
        ...
        decimal unitPrice)
    {
        ...
        var total =
            this.pricingService.CalculateTotal(unitPrice, quantity);
        ...
        decimal unitPrice)
    {
        var validationResult =
            this.validator.Validate(productId, quantity, unitPrice);

        if (!validationResult.IsValid)
        {
            throw new InvalidOrderException(
                validationResult.ErrorMessage ?? "Order validation failed");
        }
    }
}
