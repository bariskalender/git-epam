# Sports Store Application. Step 5
...
- Go to the cloned repository of the previous step `Sports Store Application. Step 4`. 

- Switch to the `step-5` branch and do a fast-forward merge according to changes from the `main` branch.
...
[Route("Admin")]
// NEW: Create AdminController for managing orders and products
...
```
`In the future, routing attributes will be used for routing support.`
...
// NEW: Create AdminNavigationMenuViewComponent for admin navigation
...
```
And then add a `Products.cshtml` Razor View file to the `Views/Admin` folder with the content shown below.
...
- To create a simple administration tool that will let you view the orders that have been received and mark them as shipped, first change the data model so that the administrator can record which orders have been shipped. Add a `Shipped` property in the `Order` class (the `Order.cs` file in the `Models` Folder).
...
public class Order
{
    ...
    [BindNever]
    // NEW: Add Shipped property to track order shipping status
...
- Add to `AdminController` class `MarkShipped` action method that will receive a POST request specifying the `Id` of an order, which is used to locate the corresponding `Order` object from the repository so that the `Shipped` property can be set to `true` and saved, and `Reset` action method that will receive a POST request specifying the `Id` of an order, which is used to locate the corresponding `Order` object from the repository so that the `Shipped` property can be set to `false` and saved.
...
public class AdminController : Controller
{
    ...
    [HttpPost]
    [Route("MarkShipped")]
    // NEW: Add MarkShipped action to mark orders as shipped
    ...
    [HttpPost]
    [Route("Reset")]
    // NEW: Add Reset action to unmark orders as shipped
...
$ git diff --staged
$ git commit -m "feat(admin): implement order management functionality"
...
public interface IStoreRepository
{
    ...
    // NEW: Add SaveProduct method for creating and updating products
    void SaveProduct(Product p);
    
    // NEW: Add CreateProduct method for creating new products
    void CreateProduct(Product p);

    // NEW: Add DeleteProduct method for removing products
...
public class EFStoreRepository : IStoreRepository
{
    ...
    // NEW: Implement CreateProduct method to add new products
    public void CreateProduct(Product product)
    {
        context.Add(product);
        context.SaveChanges();
    }
    // NEW: Implement DeleteProduct method to remove products
    public void DeleteProduct(Product product)
    {
        context.Remove(product);
        context.SaveChanges();
    }
    // NEW: Implement SaveProduct method for creating and updating products
...
public class Product
{
    ...
    // NEW: Add validation attributes for product properties
    [Required(ErrorMessage = "Please enter a product name")]
    public string Name { get; set; } = string.Empty;

    // NEW: Add required validation for product description
    [Required(ErrorMessage = "Please enter a description")]
    public string Description { get; set; } = string.Empty;

    // NEW: Add required and range validation for product price
    [Required]
    [Range(0.01, double.MaxValue, ErrorMessage = "Please enter a positive price")]
    [Column(TypeName = "decimal(8, 2)")]
    public decimal Price { get; set; }

    // NEW: Add required validation for product category
...
public class AdminController : Controller
{
    ...
    // NEW: Add Details action to display product information
...
public class AdminController : Controller
{
    ...
    // NEW: Add Edit GET action to display edit form
    ...
    [HttpPost]
    [Route("Products/Edit/{productId:long}")]
    // NEW: Add Edit POST action to process form submission
    ...
    // NEW: Add Create GET action to display create form
    ...
    [HttpPost]
    [Route("Products/Create")]
    // NEW: Add Create POST action to process form submission
...
```html
  ...
  <!-- NEW: Use _Editor partial view for editing products -->
...
<!-- NEW: Use _Editor partial view for creating products -->
...
```bash
libman install jquery@3.7.8 -d wwwroot/lib/jquery
libman install jquery-validate@1.20.1 -d wwwroot/lib/jquery-validate
...
**Important:** Always verify the latest stable versions of these libraries before using them in production. The versions shown here are current as of .NET 8 release and provide optimal compatibility and security.
...
```json
{
  ...
  "libraries": [
    ...
    {
      "provider": "cdnjs",
      "library": "jquery@3.7.8",
      "destination": "wwwroot/lib/jquery/"
    },
    {
      "provider": "cdnjs",
      "library": "jquery-validate@1.20.1",
...
```html
  ...
  <!-- NEW: Add jQuery library for client-side functionality -->
  <script src="~/lib/jquery/jquery.min.js"></script>
  <!-- NEW: Add Scripts section for validation scripts -->
...
```html
  ...
  @section Scripts
  {
    <!-- NEW: Include validation scripts for client-side validation -->
...
public class AdminController : Controller
{
    ...
    // NEW: Add Delete GET action to display delete confirmation
    public IActionResult Delete(int productId)
        => View(storeRepository.Products.FirstOrDefault(p => p.ProductId == productId));

    [HttpPost]
    [Route("Products/Delete/{productId:long}")]
    // NEW: Add DeleteProduct POST action to process deletion
...
```html
  ...
  <!-- NEW: Use _ProductInfo partial view to display product details -->
...
$ git diff --staged
$ git commit -m "feat(admin): implement product catalog management"
```
</details>
...
namespace SportsStore.Models
{
    // NEW: Create AppIdentityDbContext for ASP.NET Core Identity
...
```json
{
  ...
  "ConnectionStrings": {
    ...
    // NEW: Add Identity database connection string
...
```csharp
  ...
  // NEW: Configure Identity database context
  ...
  // NEW: Configure Identity services with Entity Framework stores
  ...
  // NEW: Enable authentication middleware
  app.UseAuthentication();
  // NEW: Enable authorization middleware
...
- To define the schema and apply it to the database, use the Entity Framework Core migrations feature. 
...
// NEW: Create IdentitySeedData class for seeding admin user
...
```csharp
  ...
  // NEW: Seed Identity database with admin user
...
```csharp
  // NEW: Add authorization requirement to AdminController
  [Authorize]
  [Route("Admin")]
  public class AdminController : Controller
  {
...
// NEW: Create LoginViewModel for user authentication
...
```
- Then add `AccountController` class (to the `AccountController.cs` file to the `SportsStore/Controllers` folder)
...
// NEW: Create AccountController for authentication and authorization
        ...
        <div class="container-fluid">
            <div class="row">
                ...
                <div class="col-2 text-right">
                    <a class="btn btn-sm btn-primary" asp-controller="Account" asp-action="Logout">Log Out</a>
                </div>
            </div>
        </div>
    </div>
...
```
- Then add `Error` action method to the `HomeController` class.
...
public class HomeController : Controller
{
    ...
    // NEW: Add Error action for custom error handling
...
```csharp
  ...
  // NEW: Add production environment check for error handling
  if (app.Environment.IsProduction())
  {
      // NEW: Configure custom error handler for production environment
  ...
  // NEW: Add error route for custom error handling
  app.MapControllerRoute(
      "error",
      "Error",
...
$ git diff --staged
$ git commit -m "feat(auth): implement ASP.NET Core Identity authentication"
```
- Push the local branch to the remote branch.

```bash
$ git push --set-upstream origin step-5

```
- Switch to the `main` branch and do a merge according to changes from the `step-5` branch.
